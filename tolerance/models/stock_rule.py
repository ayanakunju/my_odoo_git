...
class StockRule(models.Model):
    """ StockRule model is used to overwrite the stock Rule model inside the tolerance module """
    _inherit = 'stock.rule'

    def _run_buy(self, procurements):
        """ overwrite the stock rule model for adding the custom field tolerance percentage
        inside  the purchase order line  """
        procurements_by_po_domain = defaultdict(list)
        errors = []
        for procurement, rule in procurements:
            # print('_run_buy', procurement.values.get('group_id').read())
            # print('_run_buy', procurement.values.get('group_id').sale_id.filtered(lambda l: l.product_id ==
        ...
        for domain, procurements_rules in procurements_by_po_domain.items():
            ...
            for procurement in procurements:
                ...
                tolerence = procurement.values.get('group_id').sale_id.order_line.filtered(
                    lambda l: l.product_id == procurement.product_id).tolerance_percentage
                if po_line:
                    # If the procurement can be merge in an existing line. Directly
                    # write the new values on it.
                    procurement.values['tolerance_percentage'] = tolerence
                    vals = self._update_purchase_order_line(procurement.product_id,
                                                            procurement.product_qty, procurement.product_uom,
                                                            company_id,
                                                            procurement.values, po_line)

                    vals['tolerance_percentage'] = tolerence
