...
class AbstractLeaseModel(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if data.get('from_date'):
            query += " AND lm.date_start >= %s"
            params.append(data['from_date'])
        if data.get('to_date'):
            query += " AND lm.date <= %s"
            params.append(data['to_date'])
        if data.get('tenant_id'):
            query += " AND lm.tenant_id in %s"
            params.append(str(tuple(data['tenant_ids'])))
        if data.get('state'):
            query += " AND lm.state = %s"
            params.append(data['state'])
        if data.get('status'):
            query += " AND lm.status = %s"
            params.append(data['status'])
        if data.get('owner_id'):
            query += " AND pm.owner_id = %s"
